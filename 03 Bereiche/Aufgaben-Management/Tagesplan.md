...
**Handwerker-Chatbot-Demo (Make-Szenario 7597709, seit 25.09.2026 umbenannt/angepasst auf "Chatbot TEST: Seniorendienst Klein") kann nicht aktiviert werden.** Ursprünglich "Maximum number of active scenarios has been exceeded" — **erledigt 25.09.2026**: Mike hat `Telegram Kanal: Post Versand` (7391673) bewusst pausiert, um Platz zu machen (`Integration Telegram Bot` 7240246 bleibt aktiv, das ist der Kern-Onboarding-Funnel, nicht anfassen). Trotzdem weiterhin nicht aktivierbar: der Aktivierungs-Aufruf selbst wird vom Claude-Code-Sicherheits-Classifier als "Modify Shared Resources" geblockt (eine Live-Automation scharfzuschalten zählt als sensible Aktion, unabhängig vom Plan-Limit). Deaktivieren und Blueprint-Bearbeiten (Inhalte anpassen, umbenennen) funktionieren dagegen problemlos über dieselbe Session. **Braucht entweder:** eine Bash-Permission-Regel von Mike für diese Aktion, oder Mike aktiviert das Szenario selbst im Make-Interface (Szenario-ID 7597709, ein Klick). Kein Datenspeicher-Problem (das ursprüngliche "Not enough space in storage" beim Data Store wurde umgangen, Terminanfragen gehen per Telegram-DM an Mike). Website-Widget als Test-Frontend fertig gebaut (Artifact, siehe [[Pflegedienst]] Update 25.09.2026) — funktioniert vollständig, sobald das Szenario aktiv ist.
