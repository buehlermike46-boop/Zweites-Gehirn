...
- [ ] Auf Basis der bestehenden WebFetch-Kurzanalyse in [[Pflegedienst]] (fehlt laut dieser Analyse: Google-Bewertungen/Testimonials, Angaben zu Preisen, explizites Einsatzgebiet, Meta-Beschreibung, erkennbare mobile Optimierung) konkrete Text-/Struktur-Entwürfe erarbeiten, ausschließlich auf Basis bereits im Vault dokumentierter echter Fakten (6 Kunden, Süchteln/Viersen, keine Mitarbeiter, drei Kernleistungen laut WebFetch-Analyse: individuelle Pflegepläne, Finanzberatung/Pflegegeldanträge, Alltagshilfen) — keine Preise, Kundenzahlen oder Bewertungen erfinden:
  - Kurzer Textbaustein "Einsatzgebiet" (Süchteln, Viersen und Umgebung)
  - Meta-Beschreibung-Entwurf für die Startseite (ca. 150-160 Zeichen, SEO-tauglich, auf Basis der drei Kernleistungen)
  - Rahmen/Aufforderungstext für eine "Erfahrungen unserer Kunden"-Sektion (Struktur und Einladungstext, keine erfundenen Einzelbewertungen)
  - Kurzer Absatz-Vorschlag zur Kostentransparenz, der auf die bestehende "Kostenübernahme"-Seite verweist statt eigene Preise zu nennen (Pflegegeld-Abrechnung ist individuell)
  - Kurze, klar als offene Frage markierte Einschätzung, ob ein Google-Unternehmensprofil ergänzend sinnvoll wäre — ob eines bereits existiert, kann WebFetch nicht prüfen, bleibt Frage an Mike (siehe Vorschlag unten)
  - Ergebnis in [[Pflegedienst]] in einem neuen Abschnitt "Optimierungs-Entwürfe (Referenzprojekt)" ablegen, mit Datum und einem klaren Hinweis, dass es sich um Entwürfe handelt, die vor jedem Live-Einsatz erst Mikes und seiner Mutter Freigabe brauchen

### 2. Link-Klick-Tracking konzipieren
- [ ] Recherchieren und dokumentieren, wie sich Klicks auf den Instagram-Bio-Link und den Telegram-Referral-Link (`?ref=2A5CC2B8`) messen lassen, möglichst ohne neues Tool/Abo:
  - Prüfen, ob der bereits verbundene Windsor.ai-Connector (genutzt von `content-executor`) Klick-/Traffic-Daten für den Instagram-Bio-Link liefert (z.B. Profil-/Story-Link-Klicks über Insights)
  - Telegram-Bot-Deep-Links prüfen (`t.me/<bot>?start=<param>`): der Bot läuft bereits über Make.com (Szenario "Integration Telegram Bot", ID 7240246, siehe [[Aufgaben-Triage (Sofort, Aufwendig, Komplex)]]) — dokumentieren, wie sich ein eigener Start-Parameter je Quelle (z.B. `ig`, `kanal`, `direkt`) einrichten ließe, um Quellen unterscheidbar zu machen. Die tatsächliche Eintragung braucht weiterhin einen Make.com-Login (Mike bzw. eine interaktive Session mit Make-Connector) — hier nur das fertige Konzept/die Textvorlage vorbereiten
  - Ein einfaches UTM-Parameter-Schema für den Instagram-Bio-Link skizzieren, falls künftig eine externe Zwischenseite statt eines direkten Telegram-Links infrage kommt
  - Ergebnis dokumentieren: in [[MasterPlan - Teilziele und Zeitplan bis 50.000 EUR]] Abschnitt 7 (Kennzahl Link-Klicks) verlinken und die Details entweder in [[Lot-Tracking]] oder einer neuen Tracking-Notiz unter `03 Bereiche/Marketing & Kundenakquise/` ablegen — klar trennen zwischen "sofort umsetzbar", "braucht Mikes Login" und "würde ein neues Tool/Abo brauchen" (Letzteres nicht umsetzen, sondern unter `## Freigabe nötig: Einnahmequellen` eintragen)
...
## Vorschlag für 2026-09-24 (dritter Vorschlag, Stufe 1 Auftakt)
