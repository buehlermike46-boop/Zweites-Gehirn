...
**Offene Frage an Mike:** Vier gleichzeitig auf derselben Kerze zutreffende Bedingungen beibehalten (dann eventuell noch tagelang sehr wenige/keine Trades, dafür maximal streng gefiltert), oder die Zeitfenster lockern (z.B. Location-Impuls für den Rest des Tages "nachwirken" lassen statt nur eine Kerze, oder Tageskontext + Location + HA-Bounce müssen nicht exakt dieselbe Kerze treffen wie die Demand-Index-Kreuzung, sondern nur "kürzlich"/"aktuell gültig")? Das ist eine Strategie-Kalibrierungs-Entscheidung, keine, die sich aus dem Code allein beantworten lässt — an echten vergangenen Setups zu prüfen, nicht zu raten (gleiches Prinzip wie bei den anderen offenen Kalibrierungs-Punkten).

## Nächster Schritt
Build/Deploy der drei geänderten Dateien (`NqTestStrategy.cs`, `EsTageskontext.cs`, `EsLocation.cs`) bei Mike ausstehend, dann erneuter Testlauf mit den neuen Diagnose-Plots im Blick — sollte jetzt sichtbar machen, welche der vier Bedingungen tatsächlich fehlt. Danach: Mikes Entscheidung zur offenen Frage oben (Zeitfenster lockern ja/nein). Weiter offen danach: Halb-/Vollautomatik-Umschalter als echten UI-Parameter einbauen, restliches Setup/Footprint/Orderflow (Checkliste Punkt 4) einbeziehen, Footprint- und Volumenbergkanten-Schwellen an echten Setups kalibrieren, Single Prints/Range High-Low klären.
