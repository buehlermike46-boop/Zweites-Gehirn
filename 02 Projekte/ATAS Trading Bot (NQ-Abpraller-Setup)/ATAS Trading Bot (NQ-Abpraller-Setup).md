...
## Auch unbedingt geschriebener Demand Index blieb flach — letzter Test: läuft OnCalculate überhaupt? (22.09.2026)
Nach dem Umbau auf unbedingtes Schreiben (siehe oben) blieben laut Screenshot ALLE VIER Diagnose-Plots weiterhin komplett flach — auch der jetzt bedingungslose Demand Index, der bei echten Kursdaten über hunderte Kerzen zwangsläufig Bewegung zeigen müsste. Das deutet stark darauf hin, dass `OnCalculate` für diese Strategie-Instanz gar nicht erst aufgerufen wird, unabhängig vom gesamten restlichen Code.

**Letzter, unumstößlicher Test:** `NqDiagnosticsState.OnCalculateHitCount` (neuer Zähler) wird jetzt als ALLERERSTE Zeile in `NqTestStrategy.OnCalculate` hochgezählt, vor jeder anderen Berechnung. Neuer fünfter Diagnose-Indikator `NqDiagnosticsHitCount` ("RG OnCalculate-Zähler (Strategie)"). Bleibt der bei 0: `OnCalculate` läuft nicht — ein ATAS-Platform-Problem mit dieser konkreten Strategie-Instanz/diesem Chart, kein Logikfehler im Code mehr. Steigt er: dann ist etwas noch Merkwürdigeres los (Werte werden geschrieben, aber Demand Index bleibt trotzdem bei exakt 0 — praktisch ausgeschlossen bei echten Kursdaten).

## OnCalculate-Zähler bestätigt 0 — ChartStrategy.OnCalculate läuft nachweislich nie (22.09.2026)
Mikes Screenshot: "RG OnCalculate-Zähler (Strategie)" zeigt am rechten Rand "0", nach stundenlangem Live-Betrieb. Bewiesen: `NqTestStrategy.OnCalculate` wird nie aufgerufen — kein Logikfehler im Code mehr, sondern etwas Fundamentaleres. Auffällig: JEDE reine `Indicator`-Klasse in derselben DLL funktioniert nachweislich (EsTageskontext, EsLocation, alle fünf `NqDiagnostics*`) — nur die einzige `ChartStrategy`-Klasse scheint nie ihr `OnCalculate` zu bekommen, obwohl sie am 21.09. nachweislich mal funktioniert hat (echter Trade ausgelöst) und aktuell als "Aktiv" gelistet ist.

**Minimal-Test gebaut:** `NqHelloWorldStrategy.cs` (neu) — komplett unabhängig von `NqTestStrategy.cs`, keine Order-Logik, nur ein Zähler der in `OnCalculate` hochzählt. Eigener Briefkasten `NqHelloWorldState`, eigener Anzeige-Indicator `NqHelloWorldDisplay` ("RG HelloWorld Zähler (Anzeige)"). Isoliert die Frage "läuft überhaupt irgendeine ChartStrategy noch auf dieser Installation" komplett von der Komplexität in `NqTestStrategy.cs`.

**Wichtig beim Deployment:** Die Test-Strategie `NqHelloWorldStrategy` muss wie `NqTestStrategy` über die **Handelsstrategien-Funktion** aktiviert werden (nicht über den normalen Indikatoren-Button — es ist eine ChartStrategy). Nur der Anzeige-Indicator `NqHelloWorldDisplay` kommt über den Indikatoren-Button dazu.

## Endgültig bewiesen: ChartStrategy.OnCalculate läuft generell nicht mehr — kein Code-Problem (22.09.2026)
Mikes Test: selbst die komplett leere `NqHelloWorldStrategy` (keine Order-Logik, kein Bezug zu `NqTestStrategy.cs`, nur ein Zähler) blieb bei 0. Damit zweifelsfrei bewiesen: `ChartStrategy.OnCalculate` wird auf Mikes Installation aktuell generell nicht aufgerufen — unabhängig von jedem Code in diesem Projekt. Jede `Indicator`-Klasse (egal ob ES oder NQ: `EsTageskontext`, `EsLocation`, alle fünf `NqDiagnostics*`) läuft dagegen nachweislich einwandfrei.

**Neue Arbeitshypothese:** `ChartStrategy` braucht eine echte Konto-/Broker-Verbindung (Portfolio/Security), um zu laufen — ein reiner `Indicator` nicht, der braucht nur Kursdaten. Das passt zu den Rithmic-Verbindungsfehlern im Logs-Panel von heute ("Repository Connection Login Failed", siehe Abschnitt weiter oben) — bisher als Nebensache/nicht ursächlich eingestuft, jetzt Hauptverdächtiger. Würde auch erklären, warum es am 21.09. noch funktioniert hat (Order tatsächlich platziert und gefüllt) und heute nicht mehr.
