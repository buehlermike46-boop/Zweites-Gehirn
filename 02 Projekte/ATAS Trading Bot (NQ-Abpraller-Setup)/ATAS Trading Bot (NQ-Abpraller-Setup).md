...
## Nächster Schritt
Mike baut die drei geänderten Dateien (`NqDiagnosticsState.cs`, `NqDiagnostics.cs`, `NqTestStrategy.cs`), fügt "RG OnCalculate-Zähler (Strategie)" zusätzlich zum NQ-Chart hinzu. Entscheidend: zählt der überhaupt hoch? Falls nicht, ist das Problem außerhalb dieses Codes zu suchen (z.B. Strategie an falschem/verwaistem Chart-Objekt hängend, ATAS-Bug bei ChartStrategy-Callbacks) — dann macht weiteres Code-Debugging keinen Sinn mehr, sondern eher ein kompletter Neuaufbau des Charts/der Strategie von Grund auf, oder ATAS-Support kontaktieren. Rest wie gehabt: Halb-/Vollautomatik-Umschalter als echten UI-Parameter einbauen, restliches Setup/Footprint/Orderflow (Checkliste Punkt 4) einbeziehen, Footprint- und Volumenbergkanten-Schwellen sowie `HaProximityAtrFraction`/`ConfirmationWindowBars` an echten Setups kalibrieren, Single Prints/Range High-Low klären.
