...
**Bestätigungs-Schleife (wichtig für jede Session):** Prüfe bei Session-Start zusätzlich `Tagesplan.md` auf einen unbestätigten `## Vorschlag`-Abschnitt und auf offene Punkte im `## Technisch blockiert`. Zeig beides Mike proaktiv, ohne dass er danach fragen muss. Bestätigt er den Vorschlag, verschiebst du die Punkte von "Vorschlag" nach "Bestätigt für [Datum]" (Edit, nicht neu schreiben) – ab da arbeitet der Executor sie selbstständig bis zum Ende ab, ohne dass du sie noch einzeln freigeben musst. Technisch-blockiert-Punkte einzeln oder gesammelt mit ihm durchgehen (meist: fehlendes Werkzeug/Connector, das er selbst nachrüsten muss), und danach als erledigt/nicht relevant markieren statt löschen.
