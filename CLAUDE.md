...
- `aufgaben-executor` (`.claude/agents/aufgaben-executor.md`) – arbeitet ausschließlich den `## Bestätigt für [Datum]`-Abschnitt ab (Recherche, Texte, Entwürfe, Vault-Pflege, und seit 11.09.2026 auch externe Aktionen wie Senden, Posten, neue Logins, Käufe, Löschen). Läuft auch unbeaufsichtigt per Scheduled Cloud Routine.

**Volle Autonomie seit 11.09.2026 (Mikes bewusste Entscheidung, siehe [[2026-09-11]]):** Der Executor führt jede Aktion, für die er ein funktionierendes Werkzeug/Connector hat, direkt aus, ohne vorher nachzufragen – auch Senden, Posten, neue Logins, Käufe und Löschen. Es gibt dafür keine Freigabepflicht mehr, nur noch eine Log-Pflicht: jede ausgeführte externe Aktion steht mit Zeitstempel im `## Log` von `Tagesplan.md`. Wo kein Werkzeug für eine Plattform existiert (Stand 11.09.2026: Telegram, WhatsApp-Business-Automatisierung; Instagram-Profil-/Bio-Felder sind laut Meta/Windsor-API generell nicht editierbar) oder ein vorhandener Connector fehlerhaft/unzureichend berechtigt ist (z.B. Google Drive, aktuell "insufficient scope"), landet der Punkt in `## Technisch blockiert` in `Tagesplan.md` – das ist keine Freigabe-Warteschlange mehr, sondern der Hinweis, dass Mike selbst Zugriff/Connector einrichten muss, bevor der Executor drankommt. Ein Make.com-Connector existiert in der Registry, ist aber noch nicht verbunden, siehe [[2026-09-11]].

**Bestätigungs-Schleife für den Tagesplan bleibt (andere Ebene als die Ausführungsfreigabe oben):** Prüfe bei Session-Start zusätzlich `Tagesplan.md` auf einen unbestätigten `## Vorschlag`-Abschnitt und auf offene Punkte in `## Technisch blockiert`. Zeig beides Mike proaktiv, ohne dass er danach fragen muss. Bestätigt er den Vorschlag, verschiebst du die Punkte von "Vorschlag" nach "Bestätigt für [Datum]" (Edit, nicht neu schreiben) – erst dann darf der Executor sie abarbeiten. Das entscheidet nur, WELCHE Aufgaben dran sind, nicht mehr, OB einzelne Aktionen darin eine Freigabe brauchen. Punkte aus "Technisch blockiert" einzeln oder gesammelt mit ihm durchgehen und danach als erledigt/verworfen markieren statt löschen.
